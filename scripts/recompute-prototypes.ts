#!/usr/bin/env ts-node
/*
  Recompute person prototypes (up to k=3 per person) from face embeddings.
  - Picks highest-confidence face, then farthest from it, then farthest from their mean.
  - Stores normalized vectors in person_prototypes.
*/
import { prisma } from '@/lib/prisma';
import { parsePgvectorText, toPgvectorLiteral, normalizeVector } from '@/lib/vector-utils';
import { randomUUID } from 'crypto';

async function fetchPersonFaceVecs(personId: string): Promise<Array<{ id: string; vec: number[] }>> {
  const rows = await prisma.$queryRaw<Array<{ id: string; v: string }>>`
    SELECT id, embedding AS v
    FROM faces
    WHERE "personId" = ${personId} AND embedding IS NOT NULL AND ignored = false
    ORDER BY confidence DESC
    LIMIT 200
  `;
  return rows.map((r) => ({ id: r.id, vec: parsePgvectorText(r.v) })).filter((r) => r.vec.length > 0);
}

function l2(a: number[], b: number[]): number {
  let s = 0; for (let i = 0; i < a.length; i++) { const d = a[i]-b[i]; s += d*d; } return Math.sqrt(s);
}

async function recomputeForPerson(personId: string) {
  const faces = await fetchPersonFaceVecs(personId);
  // Clear existing prototypes
  await prisma.personPrototype.deleteMany({ where: { personId } }).catch(() => {});
  if (faces.length === 0) return;
  // k=1..3 based on face count
  const k = Math.min(3, Math.max(1, faces.length));
  const chosen: number[][] = [];

  // 1) highest-confidence already first in list
  chosen.push(normalizeVector(faces[0].vec));
  if (k >= 2 && faces.length >= 2) {
    // 2) farthest from chosen[0]
    let bestIdx = 1, bestDist = -1;
    for (let i = 1; i < faces.length; i++) {
      const d = l2(chosen[0], faces[i].vec);
      if (d > bestDist) { bestDist = d; bestIdx = i; }
    }
    chosen.push(normalizeVector(faces[bestIdx].vec));
  }
  if (k >= 3 && faces.length >= 3) {
    // 3) farthest from mean of first two
    const mean = chosen[0].map((v, i) => (v + chosen[1][i]) / 2);
    let bestIdx = 2, bestDist = -1;
    for (let i = 2; i < faces.length; i++) {
      const d = l2(mean, faces[i].vec);
      if (d > bestDist) { bestDist = d; bestIdx = i; }
    }
    chosen.push(normalizeVector(faces[bestIdx].vec));
  }

  // Insert
  for (const v of chosen) {
    const lit = toPgvectorLiteral(v);
    const id = 'proto_' + randomUUID().replace(/-/g, '');
    await prisma.$executeRawUnsafe(
      `INSERT INTO person_prototypes (id, "personId", embedding_vec, weight) VALUES ($1, $2, $3::vector, $4)`,
      id,
      personId,
      lit,
      1.0 / chosen.length,
    );
  }
}

async function main() {
  const people = await prisma.person.findMany({ select: { id: true } });
  const batchSize = 10; // Process 10 people at a time
  for (let i = 0; i < people.length; i += batchSize) {
    const batch = people.slice(i, i + batchSize);
    await Promise.all(batch.map(p => recomputeForPerson(p.id)));
    console.log(`Prototypes recomputed for ${Math.min(i + batchSize, people.length)}/${people.length}`);
  }
  console.log('Prototypes recompute complete');
}

main().then(() => prisma.$disconnect());
